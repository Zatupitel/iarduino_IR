...
volatile	iarduino_IR_volatile_variable_class IRVVC;

			iarduino_IR::iarduino_IR(uint8_t i, uint8_t j, bool k){
...
void		iarduino_IR::begin(){
			...
			if(IRVVC.IR_pins_NUM_OUTPUT<255){															//	если подключён светодиод
				pinMode(IRVVC.IR_pins_NUM_OUTPUT, OUTPUT);												//	переводим вывод светодиода в режим передачи
				digitalWrite(IRVVC.IR_pins_NUM_OUTPUT, IRVVC.IR_flag_SEND_INVERT);						//	выводим 0 на светодиод (или 1 если установлен флаг инверсии)
			}
...
void		iarduino_IR::send(uint32_t i, bool j){
...
//			сброс установленного протокола передачи данных
void		iarduino_IR::protocol(bool i){
			if(!i){IR_flag_SET_PROTOCOL=0;}
}
//			загрузка протокола передачи данных
void		iarduino_IR::protocol(char* i){
...
uint8_t		iarduino_IR::IR_func_PROTOCOL(){
...
bool		iarduino_IR::IR_func_DECODE(uint8_t i){
...
bool		iarduino_IR::IR_func_DECODE_BIPHASIC(uint8_t i, bool j){
			IR_uint_DATA=0; IR_uint_LENGTH=0;
...
void		iarduino_IR::IR_func_DECODE_BIPHASIC_SHIFT(bool i){
...
bool		iarduino_IR::IR_func_CHECK_NRC(){				if(length_repeat<3){return false;}
...
uint32_t	iarduino_IR::IR_func_CREATE_PACK_REPEAT(bool i, uint32_t j){ uint32_t k=0;
...
bool		iarduino_IR::IR_func_COMPARE(uint8_t i, uint8_t j, uint8_t k){
			int result = i - j; if(result<0){result*=-1;}
			return result>k? false:true;
}
//			передача пакета данных на светодиод
void		iarduino_IR::IR_func_SEND(uint32_t i, uint8_t j){
...
void		iarduino_IR::IR_func_DELAY(bool i, uint32_t j){
...
void		iarduino_IR::IR_func_TIMER2_SETREG(uint32_t i){i*=1000;
